...
$(R)/utilities/computeDeformationStress/solve: $(R)/utilities/computeDeformationStress/main.cpp  /home/sourav/Experimentation/VegaFEM-v3.1/utilities/largeModalDeformationFactory/StVKReducedInternalForcesWX.cpp ./src/SimulatorApp.cpp ./src/FEASolve.cpp ./src/Optimizer.cpp ./src/mesh.cpp $(DISPLAYOBJ_LIB_FILENAMES) $(DISPLAYOBJ_HEADER_FILENAMES)
	$(CXXLD) $(LDFLAGS) $(INCLUDE) $(GLUI_INCLUDE) $(BLASLAPACK_INCLUDE) -I/usr/include/eigen3 $(DISPLAYOBJ_OBJECTS) $^ $(DISPLAYOBJ_LINK) -std=c++11 -lThea -lboost_system -lboost_thread -lboost_filesystem -lboost_program_options -lm -Wl,-rpath,$(GLUI_DIR)/lib $(IMAGE_LIBS) `$(WX_CONFIG) --cxxflags --libs core,base,gl` -o $@; cp $@ $(R)/utilities/bin/
